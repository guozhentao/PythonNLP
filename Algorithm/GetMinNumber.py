        ...
        if len(mylist) < 2:
            ...
            for elem in mylist[1:]:
                if midValue+elem >elem+midValue:
                    ...
                    greater.append(elem)
        return quickSort(less) + [midValue] + quickSort(greater)
    if not numbers or len(numbers)==0:
        return ''
    strnumbers = [str(elem) for elem in numbers]    # 将intlist转为strlist
    res = quickSort(strnumbers)
    return int(''.join(res))